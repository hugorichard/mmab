...
K = 3  # Number of arms
mu = np.array([0.01, 0.01, 0.99])  # Reward of each arm
p = 0.3  # Probability that a player is active at each round
T = 200000  # Number of rounds

verbose = True

names_agents = [
    ("3 on the best arm", Naive(np.array([0, 0, 3]))),
    ("ETC", ETC(M, K, p, T)),
...
plt.xlabel("Time $t$")
plt.legend()
plt.savefig("../figures/0_players_small.pdf", bbox_inches="tight")
