...
fig, axes = plt.subplots(nrows=2, sharex=True, figsize=(6, 2))
for ax in axes:
...
for a, (plotm, plot_h, plot_l) in enumerate([(pm, ph, pl), (pm2, ph2, pl2)]):
    ...
    for i, name in enumerate(names):
        if i == 0:
            continue
        axes[a].plot(H, plot_opt- plotm[i], label=name)
...
plt.legend(ncols=3, loc="upper right", bbox_to_anchor=(1.05, 3.5), frameon=False)
plt.savefig("../figures/figure1.pdf", bbox_inches="tight")
plt.close()
import os
os.system("pdfcrop ../figures/figure1.pdf ../figures/figure1.pdf")
